...
def create_table(conn, cursor):
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS oe_compatibility_oepkgs (IDX SERIAL PRIMARY KEY, summary VARCHAR(800), repoType VARCHAR(30), openeuler_version VARCHAR(20), rpmPackUrl VARCHAR(300), srcRpmPackUrl VARCHAR(300), name VARCHAR(100), arch VARCHAR(20), id VARCHAR(30), rpmLicense VARCHAR(600), version VARCHAR(80), osVer VARCHAR(20));")
    conn.commit()
...
def write_data(data, conn, cursor, sql):
    data_list = []
    for item in data:
        data_list.append((item['summary'], item['repoType'], item['os'], item['rpmPackUrl'], item['srcRpmPackUrl'],
                          item['name'], item['arch'], item['id'], item['rpmLicense'], item['version'], item['osVer']))
