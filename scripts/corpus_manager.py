...
from scripts.drop_all_table import drop_all_tables
from scripts.init_all_table import create_db_and_tables
from scripts.upload_courpus import upload_corpus
from scripts.delete_courpus import delete_corpus
from scripts.query_corpus import query_corpus
from scripts.stop_all_job import stop_embedding_job
from scripts.init_asset import init_asset
from scripts.change_doucument_to_para import change_document_to_para
...
def init_args():
    ...
    parser.add_argument("--method", type=str, required=True, choices=['init_pg', 'init_corpus_asset', 'clear_pg',
                                                                      'up_corpus', 'del_corpus', 'query_corpus',
                                                                      'stop_embdding_jobs'], help='''
                                                                     脚本使用模式，有初始化数据库、初始化语料资产、
    ...
    parser.add_argument("--corpus_dir", type=str, default='/rag-service/scripts/docs', required=False, help="待上传语料所在路径")
    parser.add_argument("--corpus_chunk", type=int, default=1024, required=False, help="语料切割尺寸")
    parser.add_argument("--corpus_name", type=str, required=False, help="待查询或者待删除语料名")
