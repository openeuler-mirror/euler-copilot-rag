...
class Security:
    ...
    def _get_root_key(half_key1: str) -> bytes:
        half_key2 = "hflkgh82f688h"
        key = (half_key1 + half_key2).encode("utf-8")
        half_key3 = b'hflkgh82f688hvdsgdf'
    ...
    def _generate_encrypted_work_key(half_key1: str) -> (str, str):
        ...
        bin_encrypted_work_key = Security._root_encrypt(
            bin_root_key, bin_encrypted_work_key_iv, bin_work_key)
        encrypted_work_key = base64.b64encode(
            bin_encrypted_work_key).decode("ascii")
        encrypted_work_key_iv = base64.b64encode(
            bin_encrypted_work_key_iv).decode("ascii")
    ...
    def _get_work_key(half_key1: str, encrypted_work_key: str, encrypted_work_key_iv: str) -> bytes:
        bin_root_key = Security._get_root_key(half_key1)
        bin_encrypted_work_key = base64.b64decode(
            encrypted_work_key.encode("ascii"))
        bin_encrypted_work_key_iv = base64.b64decode(
            encrypted_work_key_iv.encode("ascii"))
    ...
    def _root_encrypt(key: bytes, encrypted_iv: bytes, plaintext: bytes) -> bytes:
        encryptor = Cipher(algorithms.AES(key), modes.GCM(
            encrypted_iv), default_backend()).encryptor()
    ...
    def _encrypt_plaintext(half_key1: str, encrypted_work_key: str, encrypted_work_key_iv: str,
        ...
        bin_work_key = Security._get_work_key(
            half_key1, encrypted_work_key, encrypted_work_key_iv)
        salt = f"{half_key1}{plaintext}"
        plaintext_temp = salt.encode("utf-8")
        del plaintext
        del salt
        bin_encrypted_iv = secrets.token_bytes(16)
        bin_encrypted_plaintext = Security._root_encrypt(
            bin_work_key, bin_encrypted_iv, plaintext_temp)
        encrypted_plaintext = base64.b64encode(
            bin_encrypted_plaintext).decode("ascii")
    ...
    def _decrypt_plaintext(half_key1: str, encrypted_work_key: str, encrypted_work_key_iv: str,
        ...
        bin_work_key = Security._get_work_key(
            half_key1, encrypted_work_key, encrypted_work_key_iv)
        bin_encrypted_plaintext = base64.b64decode(
            encrypted_plaintext.encode("ascii"))
        bin_encrypted_iv = base64.b64decode(encrypted_iv.encode("ascii"))
        plaintext_temp = Security._root_decrypt(
            bin_work_key, bin_encrypted_iv, bin_encrypted_plaintext)
    ...
    def encrypt(plaintext: str) -> (str, dict):
        ...
        :return:
        """
        seed = "1243254366fhgdklghkdfhghjiodh4r589346984"

        half_key1 = "".join(random.sample(seed, 16))
