...
class CorpusManager():
    ...
    @staticmethod
    def is_rag_busy(language,engine):
        cnt = None
        try:
            ...
            if language=='zh':
                ...
                print(f"Query corpus upload task failed due to: {e}")
                CorpusManager.logger.error(f"Query corpus upload task failed due to: {e}")
            return True
        return cnt != 0

    @staticmethod
    def upload_files(language,upload_file_paths: List[str], engine, rag_url, kb_name, kb_asset_name) -> bool:
        if language=='zh':
            CorpusManager.logger.info(f'用户尝试上传以下片段{str(upload_file_paths)}')
        else:
            CorpusManager.logger.info(f'The user attempted to upload the following fragment{str(upload_file_paths)}')
        upload_files_list = []
        ...
        while CorpusManager.is_rag_busy(language,engine):
            if language=='zh':
                print('等待任务完成中')
            else:
        ...
        if res.status_code == 200:
            if language=='zh':
                ...
                print(f'Uploaded fragment {str(upload_file_paths)} successfully')
                CorpusManager.logger.info(f'Uploaded fragment {str(upload_file_paths)} successfully')
            return True
        else:
            if language=='zh':
                ...
                print(f'Uploaded fragment {str(upload_file_paths)} failed')
                CorpusManager.logger.info(f'Uploaded fragment {str(upload_file_paths)} failed')
            return False

    @staticmethod
    def upload_corpus(language,database_url, rag_url, kb_name, kb_asset_name, corpus_dir, up_chunk):
        try:
            ...
            if language=='zh':
                ...
                CorpusManager.logger.error(f'Database engine initialization failed due to reason {e}')
            raise e
            ...
            for file_name in files:
                ...
                if index == up_chunk:
                    ...
                    if language=='zh':
                        ...
                        if upload_res:
                            print(f'Batch {batch_count} file fragment uploaded successfully')
                        else:
                            print(f'Batch {batch_count} file fragment uploaded failed')
                    file_paths.clear()
            if index != 0:
                batch_count += 1
                upload_res = CorpusManager.upload_files(language,file_paths, engine, rag_url, kb_name, kb_asset_name)
                if language=='zh':
                    if upload_res:
                        print(f'第{batch_count}批次文件片段上传成功')
                    else:
    ...
    @staticmethod
    def delete_corpus(language,database_url, kb_name, kb_asset_name, corpus_name):
            ...
            with sessionmaker(bind=engine)() as session:
                ...
                if kb_count == 0:
                    if language=='zh':
                        ...
                        print(f'The deletion of corpus {corpus_name} in knowledge base asset {kb_asset_name} under knowledge base {kb_name} failed, and knowledge base{kb_name} does not exist')
                        CorpusManager.logger.error(f'The deletion of corpus {corpus_name} in knowledge base asset {kb_asset_name} under knowledge base {kb_name} failed, and knowledge base {kb_name} does not exist')
                    return
                ...
                if kb_asset_count == 0:
                    if language=='zh':
                        ...
                        print(f'The deletion of corpus {corpus_name} in knowledge base asset {kb_asset_name} under knowledge base {kb_name} failed, and knowledge base asset {kb_name} does not exist')
                        CorpusManager.logger.error(f'The deletion of corpus {corpus_name} in knowledge base asset {kb_asset_name} under knowledge base {kb_name} failed, and knowledge base asset {kb_name} does not exist')
                    return
                ...
                pattern = '^'+os.path.splitext(corpus_name)[0] + r'_paragraph\d+\.docx$'
                query = (
                ...
                if len(corpus_name_list) == 0:
                    if language=='zh':
                        ...
                        print(f'Failed to delete corpus, no relevant corpus was found in the database: {corpus_name}')
                        CorpusManager.logger.info(f'Failed to delete corpus, no relevant corpus was found in the database: {corpus_name}')
                    return
                ...
                if language=='zh':
                    ...
                    CorpusManager.logger.info(f'Successfully deleted corpus:{corpus_name}')
                return
        except Exception as e:
            if language=='zh':
                ...
                print(f'Failed to delete corpus due to reasons {e}')
                CorpusManager.logger.error(f'Failed to delete corpus due to reasons {e}')
            raise e

    @staticmethod
    def query_corpus(language,database_url, kb_name, kb_asset_name, corpus_name):
        try:
            engine = create_engine(
                database_url,
                pool_size=20,
        ...
        try:
            ...
            if language=='zh':
                ...
                print(f'Corpus query failed due to reasons {e}')
                CorpusManager.logger.error(f'Corpus query failed due to reasons {e}')
            raise e
        t_dict = {}
        for i in range(len(corpus_name_list)):
            try:
                ...
                if language=='zh':
                    ...
                    print(f'Fragment name conversion failed due to {e}')
                    CorpusManager.logger.error(f'Fragment name conversion failed due to {e}')
                continue
        corpus_name_list = []
    ...
    @staticmethod
    def stop_corpus_uploading_job(language,database_url):
        try:
            engine = create_engine(
                database_url,
                pool_size=20,
        ...
        try:
            ...
            if language=='zh':
                ...
                print(f'An accident occurred while stopping the corpus upload task：{e}')
                CorpusManager.logger.error(f'An accident occurred while stopping the corpus upload task：{e}')
            raise e
    ...
    @staticmethod
    def get_uploading_para_cnt(language,database_url, kb_name, kb_asset_name, para_name_list):
        try:
            engine = create_engine(
                database_url,
                pool_size=20,
        ...
        try:
            ...
            if language=='zh':
                ...
                print(f'An unexpected incident occurred while counting the number of uploaded segments of the corpus:{e}')
                CorpusManager.logger.error(f'An unexpected incident occurred while counting the number of uploaded segments of the corpus:{e}')
            raise e
