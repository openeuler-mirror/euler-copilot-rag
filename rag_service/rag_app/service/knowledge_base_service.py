...
async def get_llm_answer(req: QueryRequest) -> LlmAnswer:
    ...
    res = await select_llm(req).nonstream(req=req, prompt=prompt_template_dict['LLM_PROMPT_TEMPLATE'].format(context=query_context))
    if req.fetch_source:
        return LlmAnswer(
...
async def get_llm_stream_answer(req: QueryRequest) -> str:
    ...
    llm=select_llm(req)
    return llm.stream(
        req=req, prompt=prompt_template_dict['LLM_PROMPT_TEMPLATE'].format(context=query_context))
